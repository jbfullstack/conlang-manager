...
api.interceptors.request.use((config) => {
	...
	if (process.env.NEXT_PUBLIC_DEBUG_HMAC === 'true') {
		console.log('[HMAC FRONT]', { method, url, timestamp, bodyString: typeof body === 'string' ? body : JSON.stringify(body) });
	}
...
export const fetchPostProperties = async (payload: any) => await fetch('/api/properties', 'POST', payload);
...
export const updateProperties = (id: string, payload: any) => api.put(`/api/properties/${id}`, payload);
...
export const fetchPostCompositions = async (payload: any) => await fetch('/api/compositions', 'POST', payload);
...
export const fetchPostSearchReverse = async (payload: any) => await fetch('/api/search-reverse', 'POST', payload);
export const fetchPostAnalyzeComposition = async (payload: any) => await fetch('/api/analyze-composition', 'POST', payload);
